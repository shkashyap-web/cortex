...
import type { ObservabilityMetric, CortexEvent, AgentPluginMetadata } from '@/types';

// workflowEngine is currently surfaced only via the static
// `platformServices` health list below (unchanged UI behaviour); kept
// imported so this page continues to reflect Workflow Engine as a
// live dependency per the Milestone 2 services architecture.
void workflowEngine;
...
export default function ObservabilityPage() {
  ...
  const latencyMetrics = metrics.filter((m: ObservabilityMetric) => m.type === 'LATENCY');
  const errorMetrics = metrics.filter((m: ObservabilityMetric) => m.type === 'ERROR');
  const avgLatency = latencyMetrics.length > 0
    ? (latencyMetrics.reduce((s: number, m: ObservabilityMetric) => s + (m.durationMs || 0), 0) / latencyMetrics.length).toFixed(1)
    : '—';

  const eventHistory: ReadonlyArray<CortexEvent> = eventBus.getRecentEvents();
  const agentStatuses: AgentPluginMetadata[] = agentRegistryService.getAllAgents();
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 font-mono">
        ...
        <div className="border border-zinc-800 bg-zinc-950 rounded p-5 space-y-3">
          ...
          <div className="space-y-1.5">
            {agentStatuses.map((agent: AgentPluginMetadata) => (
